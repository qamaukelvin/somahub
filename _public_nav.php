...
<header class="somahub-public-header">
  <div class="somahub-public-navbar">
    ...
    <nav><ul id="somahubPublicNavlinks">
      ...
      <li><a href="<?= $navRoot ?>/search.php">Search</a></li>
    ...
    <button class="somahub-public-menu-toggle" id="somahubPublicMenuToggle" aria-label="Menu" aria-expanded="false">☰</button>
  </div>
</header>
<style>
  *{box-sizing:border-box;}
  body{margin:0;}
  .somahub-public-header{position:sticky;top:0;left:0;right:0;width:100%;z-index:1000;background:rgba(247,242,231,0.96);backdrop-filter:blur(6px);border-bottom:1px solid var(--line,#E5DFCC);}
  .somahub-public-navbar{display:flex;align-items:center;justify-content:space-between;padding:16px 24px;max-width:1120px;margin:0 auto;width:100%;}
  .somahub-public-brand{display:flex;align-items:center;gap:8px;font-weight:800;font-size:1.15rem;color:var(--teal-deep,#0A3A3E);text-decoration:none;flex-shrink:0;}
  ...
  .somahub-public-menu-toggle{display:none;background:none;border:none;font-size:1.4rem;cursor:pointer;color:var(--teal-deep,#0A3A3E);flex-shrink:0;line-height:1;padding:4px 6px;}
  @media(max-width:820px){
    .somahub-public-navbar nav ul{display:none;}
    .somahub-public-menu-toggle{display:block;}
    .somahub-public-navbar nav ul.open{display:flex;flex-direction:column;position:absolute;top:60px;left:0;right:0;background:var(--sand,#F7F2E7);padding:20px 24px;border-bottom:1px solid var(--line,#E5DFCC);gap:16px;align-items:flex-start;z-index:2000;}
  }
</style>
<script>
(function() {
  var toggleBtn = document.getElementById('somahubPublicMenuToggle');
  var menu = document.getElementById('somahubPublicNavlinks');
