...
if (is_premium_locked($schoolPlanRow)) {
    die('Attendance is a Premium-plan feature, or your paid term has ended. <a href="checkout.php">Upgrade now</a> to reactivate.');
}

$error = '';
$success = '';

if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_FILES['attendance_file']['tmp_name'])) {
    $termLabel = trim($_POST['term_label'] ?? '');
    $tmpPath = $_FILES['attendance_file']['tmp_name'];
    ...
    if (!$termLabel) {
        ...
        $storeDir = __DIR__ . '/../uploads/attendance/' . $user['school_id'] . '/';
        if (!is_dir($storeDir)) mkdir($storeDir, 0755, true);
        $storedName = 'attendance_' . time() . '.csv';
        move_uploaded_file($tmpPath, $storeDir . $storedName);
        $storedPath = '/uploads/attendance/' . $user['school_id'] . '/' . $storedName;

        $uploadStmt = $db->prepare("
            INSERT INTO attendance_uploads (school_id, uploaded_by_user_id, term_label, original_filename, stored_path, status)
            VALUES (?, ?, ?, ?, ?, 'processing')
        ");
        $uploadStmt->execute([$user['school_id'], $user['id'], $termLabel, $origName, $storedPath]);
        $uploadId = $db->lastInsertId();

        // Expected columns: admission_no, student_name (required), days_present, days_absent, days_late (optional)
        ...
        if ($missing) {
            $error = 'Missing required column(s): ' . implode(', ', $missing) . '. Required columns: admission_no, student_name (plus days_present, days_absent, days_late — optional).';
            $db->prepare("UPDATE attendance_uploads SET status='failed' WHERE id=?")->execute([$uploadId]);
        } else {
            $insertRow = $db->prepare("
                INSERT INTO attendance_rows (attendance_upload_id, school_id, admission_no, student_name, days_present, days_absent, days_late)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            ");
            $count = 0;
            ...
            foreach ($rows as $row) {
                ...
                $insertRow->execute([
                    ...
                    $data['days_present'] !== '' ? (int)$data['days_present'] : null,
                    $data['days_absent'] !== '' ? (int)$data['days_absent'] : null,
                    $data['days_late'] !== '' ? (int)$data['days_late'] : null,
                ]);
                $count++;
            }

            $db->prepare("UPDATE attendance_uploads SET status='ready', row_count=? WHERE id=?")
...
$uploads = $db->prepare("SELECT * FROM attendance_uploads WHERE school_id=? ORDER BY uploaded_at DESC");
...
<main class="wrap">
  <h1>Attendance</h1>
  <p class="sub">Upload term attendance so it appears on each student's combined report.</p>

  <div class="hint">
    <strong>File format:</strong> CSV file with columns <code>admission_no</code>, <code>student_name</code> (required),
    and optionally <code>days_present</code>, <code>days_absent</code>, <code>days_late</code>.<br>
  ...
  <form method="POST" enctype="multipart/form-data">
    ...
    <input type="file" name="attendance_file" accept=".csv" required>
