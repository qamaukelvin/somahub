...
if (!$user || !in_array($user['role'], ['school_owner', 'school_editor'])) {
    header('Location: ../get-started.php?mode=login');
    exit;
}
if (!empty($user['school_id'])) {
    // Already has a school - nothing to set up, send them to their real dashboard.
    ...
    if (!$schoolName || !$slug) {
        ...
        if ($check->fetch()) {
            ...
            try {
                ...
                // Attach this school to the already-logged-in user's account
                $db->prepare("UPDATE users SET school_id = ? WHERE id = ?")->execute([$schoolId, $user['id']]);
...
<title>Set Up Your School - Somahub</title>
...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0;}
  ...
  .step-indicator{font-size:0.78rem;font-weight:700;color:var(--teal);text-transform:uppercase;letter-spacing:0.04em;margin-bottom:6px;}
...
<main class="wrap">
  <div class="step-indicator">Step 2 of 3 - School Details</div>
  <h1>Now, tell us about your school</h1>
  <p class="sub">Pick a plan, choose a theme, and pick your web address.</p>

  <div class="card">
    <?php if ($error): ?><div class="error"><?= htmlspecialchars($error) ?></div><?php endif; ?>

    <form method="POST" id="schoolSetupForm" class="loader-on-submit">
  ...
  <p class="flow-note">Next, you'll edit your site content and upload verification documents in your dashboard. Your site is live immediately for a preview window, and stays live once verified.</p>
</main>
...
document.getElementById('schoolSetupForm').addEventListener('submit', (e) => {
    if (!isAvailable) {
        e.preventDefault();
